...
export default function Cuenta() {
  ...
  const { darkMode, setDarkMode, serviciosUsuario, puntos, historialPagos, totalMensual, fechaVencimiento, fechaPago, usuarioActual } = useApp()
  ...
  const serviciosActivos = serviciosUsuario ? serviciosUsuario.filter(s => s.estado === 'Activo') : []
    ...
    <div style={{ background: c.bg, minHeight: '100vh', paddingBottom: '70px', transition: 'all 0.3s' }}>
      ...
      <div style={{ background: c.header, padding: '20px 20px 40px', color: 'white', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          <div>
            <p style={{ fontSize: '20px', fontWeight: '800' }}>{usuarioActual.nombre}</p>
            <p style={{ fontSize: '13px', opacity: 0.8 }}>{usuarioActual.email}</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px' }}>
              <span style={{ background: 'rgba(255,255,255,0.2)', padding: '2px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600' }}>{usuarioActual.nivel}</span>
              <span style={{ background: 'rgba(255,255,255,0.2)', padding: '2px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600' }}>{usuarioActual.cliente}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '15px', marginTop: '-20px' }}>
        ...
        <div style={{ background: c.card, borderRadius: '20px', padding: '18px', marginBottom: '15px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ background: c.rowBg, borderRadius: '12px', padding: '12px', marginBottom: '12px', border: `1px solid ${c.border}` }}>
            ...
            {serviciosUsuario.filter(s => s.estado === 'Activo').map((s, i) => (
        ...
        <div style={{ background: c.card, borderRadius: '20px', padding: '18px', marginBottom: '15px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
          ...
          {usuarioActual.direcciones.map((d, i) => (
