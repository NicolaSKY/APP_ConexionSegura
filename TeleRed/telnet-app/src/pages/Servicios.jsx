...
export default function Servicios() {
  ...
  const {darkMode, setDarkMode, serviciosUsuario, agregarServicio, reactivarServicio,notificacionesGlobales, marcarNotificacionLeida, marcarTodasNotificacionesLeidas,} = useApp()
  ...
  const idsActivos = serviciosUsuario.map(s => s.id)
  const serviciosFaltantes = todoServicios.filter(s => !idsActivos.includes(s.id))
  const totalMes = serviciosUsuario.filter(s => s.estado === 'Activo').reduce((acc, s) => acc + s.precio, 0)
  const activos = serviciosUsuario.filter(s => s.estado === 'Activo').length
  const suspendidos = serviciosUsuario.filter(s => s.estado === 'Suspendido').length
      ...
      <div className="servicios-header" style={{ background: c.header }}>
        <div className="servicios-header-top">
          <div className="servicios-logo-row">
            ...
            <div>
              <h1 className="servicios-logo-title">TeleRed</h1>
        ...
        <div className="servicios-notif-overlay" onClick={() => setShowNotif(false)}>
          <div className="servicios-notif-panel" style={{ background: c.card }} onClick={e => e.stopPropagation()}>
            <div className="servicios-notif-header" style={{ borderBottom: `1px solid ${c.border}` }}>
              ...
              <button className="servicios-notif-mark-all" onClick={marcarTodasNotificacionesLeidas}>Marcar todas leídas</button>
            </div>
        ...
        <div className="servicios-resumen-card" style={{ background: c.speedCard }}>
          ...
          <div className="servicios-resumen-grid">
            {[
              ...
              { label: 'Total', value: serviciosUsuario.length, icon: '📋' },
            ].map((s, i) => (
          ...
          <div className="servicios-agregar-panel" style={{ background: c.bg }} onClick={e => e.stopPropagation()}>
            ...
            {exitoAgregar ? (
              <div className="servicios-modal-exito" style={{ padding: '40px 20px' }}>
                ...
                <p className="servicios-modal-exito-desc" style={{ color: c.subtext }}>¡Bienvenido a TeleRed Premium!</p>
              </div>
            ) : (
