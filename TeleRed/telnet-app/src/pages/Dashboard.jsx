...
export default function Dashboard() {
  ...
  const [velocidad, setVelocidad] = useState(usuarioActual?.velocidad ?? 150)
  const [actualizando, setActualizando] = useState(false)
  const [bajada, setBajada] = useState(Math.floor((usuarioActual?.velocidad ?? 150) * 0.1))
  const [subida, setSubida] = useState(Math.floor((usuarioActual?.velocidad ?? 150) * 0.07))

  const noLeidas = notificacionesGlobales.filter(n => !n.leida).length
  const ticketsRecientes = ticketsUsuario.slice(0, 3)
    ...
    <div className="dashboard-container" style={{ background: c.bg }}>
      ...
      <div className="dashboard-header" style={{ background: c.header }}>
        <div className="dashboard-header-top">
          <div className="dashboard-logo-row">
            ...
            <div>
              <h1 className="dashboard-logo-title">TeleRed</h1>
        ...
        <div className="dashboard-greeting">
          ...
          <p className="dashboard-greeting-name">{usuarioActual.nombre} 👋</p>
        </div>
      </div>
        ...
        <div className="dashboard-notif-overlay" onClick={() => setShowNotif(false)}>
          <div className="dashboard-notif-panel" style={{ background: c.card }} onClick={e => e.stopPropagation()}>
            <div className="dashboard-notif-header" style={{ borderBottom: `1px solid ${c.border}` }}>
              ...
              <button className="dashboard-notif-mark-all" onClick={marcarTodasNotificacionesLeidas}>
                Marcar todas leídas
              </button>
            </div>
            <div className="dashboard-notif-list">
              {notificacionesGlobales.map(n => (
                <div key={n.id}
      ...
      <div className="dashboard-content">
        ...
        {usuarioActual.señal === 'baja' && (
          <div onClick={() => navigate('/monitoreo')}
            style={{ background: darkMode ? '#2d0a0a' : '#ffebee', border: '2px solid #f44336', borderRadius: '15px', padding: '15px', marginBottom: '15px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ background: '#f44336', borderRadius: '12px', width: '44px', height: '44px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', flexShrink: 0 }}>📶</div>
            <div style={{ flex: 1 }}>
              <p style={{ fontWeight: '700', color: '#c62828', fontSize: '14px' }}>⚠️ Señal baja detectada</p>
              <p style={{ fontSize: '12px', color: darkMode ? '#8892b0' : '#666' }}>Velocidad: {usuarioActual.velocidad} Mbps · Toca para ver detalles y resolver</p>
            </div>
            <span style={{ color: '#f44336', fontSize: '22px' }}>›</span>
          </div>
        )}
