...
const respuestasBot = {
  ...
  'default': '🤖 Entiendo tu consulta. Un agente especializado te contactará en menos de 5 minutos. También puedes llamarnos al 📞 0800-TeleRed (gratuito).',
}
    ...
    setTimeout(() => {
      ...
      if (lower.includes('agente') || lower.includes('humano') || lower.includes('persona')) {
        ...
        setTimeout(() => {
          ...
          agregarMensaje({ de: 'agente', texto: '¡Hola Carlos! Soy Carlos Quispe, agente de soporte TeleRed. Vi tu conversación, ¿en qué puedo ayudarte?', hora: hora() })
        }, 800)
      } else {
    ...
    <div style={{ background: c.bg, height: '100vh', display: 'flex', flexDirection: 'column', transition: 'all 0.3s' }}>
      ...
      <div style={{ background: c.header, padding: '15px 20px', color: 'white', boxShadow: '0 4px 20px rgba(0,0,0,0.3)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: '15px', fontWeight: '800' }}>{agenteActivo ? 'Carlos Quispe' : 'Asistente TeleRed'}</p>
              ...
              <div style={{ maxWidth: '75%' }}>
                {!esUser && (
                  <p style={{ fontSize: '10px', color: c.subtext, marginBottom: '4px', fontWeight: '600' }}>
                    {esAgente ? 'Carlos Quispe · Agente' : 'Asistente TeleRed'}
                  </p>
                )}
                <div style={{
                  padding: '12px 15px',
